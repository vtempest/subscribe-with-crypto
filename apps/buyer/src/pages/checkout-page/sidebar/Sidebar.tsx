...
const Sidebar: FC = () => {
  ...
  return (
    <Grid
      ...
      sm={4}
      md={4}
      sx={{ width: '100%', position: 'fixed', height: isSmOrUp ? '100%' : '15%' }}
    >
      <Box
        sx={{
          display: 'flex',
      ...
      >
        ...
        >
          {isSmOrUp && (
            <Typography variant="h5">Manage your {vendor} Billing Settings</Typography>
          )}
          <Box
            sx={{
              display: 'flex',
          ...
          {isSmOrUp && (
            <Typography sx={{ mt: 'auto' }}>Powered by crypto-subscribe</Typography>
          )}
        </Box>
      </Box>
    </Grid>
